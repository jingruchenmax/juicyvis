...
import { playHoverSound, playClickSound, playGrabSound, playReleaseSound, playZoomSound, playWhooshSound } from '../utils/soundUtils'
import './Globe.css'
...
function GlobeJuicy({ width = 975, height = 610 }: GlobeProps) {
  ...
  const selectedCountriesRef = useRef<Set<number>>(new Set())
  ...
  useEffect(() => {
    ...
    const zoom = d3.zoom<SVGSVGElement, unknown>()
      ...
      .on('start', () => {
        ...
        d3.select('#vignetteOverlay').transition().duration(100).attr('opacity', '0.25')
      })
      .on('zoom', zoomed)
      .on('end', () => {
        playReleaseSound()
        ...
        // Remove vignette with smooth transition
    ...
    function renderCountries() {
      countriesGroup.selectAll('path')
        ...
        .on('mouseout', function (_event: any, d: any) {
          const isSelected = selectedCountriesRef.current.has(d.id)
        ...
        .attr('opacity', (d: any) => selectedCountriesRef.current.has(d.id) ? 1 : 0.85)
        .attr('filter', (d: any) => selectedCountriesRef.current.has(d.id) ? 'drop-shadow(0 0 8px rgba(180,180,180,0.5))' : 'none')
        .append('title')
        .text((d: any) => {
          const countryCode = idToCode[d.id]
          const mortality = countryCode ? mortalityData[countryCode] : undefined
      ...
      // Custom granular wheel zoom handler for smooth, dynamic zooming
      svg.on('wheel', function(event: WheelEvent) {
        event.preventDefault()
        
        // Even finer multipliers for ultra-smooth zooming across all input methods
        // LINE mode: 0.02 (trackpad: slow but responsive)
        // PIXEL mode: 0.001 (mouse wheel: finest granularity - ~100 divisions per 8x zoom)
        // PAGE mode: 0.5 (page scrolling: medium speed)
        const multiplier = event.deltaMode === 1 ? 0.02 : event.deltaMode === 0 ? 0.001 : 0.5
        const deltaY = -event.deltaY * multiplier
        
        // Get current zoom and apply new scale with exponential smoothing
        const currentTransform = d3.zoomTransform(svg.node() as SVGSVGElement)
        const newK = Math.max(1, Math.min(8, currentTransform.k * Math.exp(deltaY)))
        
        // Apply zoom immediately without transition for responsive feel
        svg.call(
    ...
    // Add vignette shadow overlay AFTER everything is rendered - add to SVG level for fixed positioning
    svg.append('rect')
      .attr('x', 0)
      .attr('y', 0)
      .attr('width', width)
    ...
    function clicked(event: MouseEvent, d: any) {
      ...
      // Toggle selection
      const newSelected = new Set(selectedCountriesRef.current)
      if (newSelected.has(d.id)) {
        newSelected.delete(d.id)
      } else {
        newSelected.add(d.id)
      }
      selectedCountriesRef.current = newSelected
      ...
      countriesGroup.selectAll('path')
        .transition()
        .attr('opacity', (d: any) => newSelected.has(d.id) ? 1 : 0.85)
        .attr('filter', (d: any) => newSelected.has(d.id) ? 'drop-shadow(0 0 8px rgba(180,180,180,0.5))' : 'none')
    ...
    function zoomed(event: any) {
      ...
      // Check if it's panning (zoom level hasn't changed)
      if (isPanningRef.current && Math.abs(newZoomLevel - lastZoomLevelRef.current) < 0.01) {
        const now = Date.now()
        // Play whoosh sound periodically during panning (not too frequently)
        if (now - lastPanTimeRef.current > 150) {
          playWhooshSound()
          lastPanTimeRef.current = now
        }
        
        // Apply vignette effect during panning
        d3.select('#vignetteOverlay').attr('opacity', '0.3')
      }
      
      // Detect zoom direction and provide feedback
      if (newZoomLevel > lastZoomLevelRef.current) {
        // Zooming in
        playZoomSound()
      } else if (newZoomLevel < lastZoomLevelRef.current && lastZoomLevelRef.current > 1.1) {
        // Zooming out
        playZoomSound()
      }
  ...
  const clearSelection = () => {
    selectedCountriesRef.current = new Set()
  ...
  const zoomIn = () => {
    ...
    // Use finer increments (20% instead of 50%) for more dynamic zooming
    svg.transition()
      .duration(400)
      .ease(d3.easeCubicOut)
      .call(zoomBehaviorRef.current as any, d3.zoomIdentity.scale(Math.min(currentZoom * 1.2, 8)))
  }
  ...
  const zoomOut = () => {
    ...
    svg.transition()
      .duration(400)
      .ease(d3.easeCubicOut)
      .call(zoomBehaviorRef.current as any, d3.zoomIdentity.scale(Math.max(currentZoom / 1.2, 1)))
  }
        ...
        <div style={{ marginBottom: '10px' }}>
          {hoveredCountry ? (
            <div>
              ...
              {selectedCountriesRef.current.has(hoveredCountry.id) && <span style={{ marginLeft: '10px', color: '#00a0ff' }}>★ Selected</span>}
            </div>
          ) : (
        ...
        <div style={{ fontSize: '12px', color: '#666' }}>
          ...
          <div style={{ marginTop: '15px', paddingTop: '15px', borderTop: '1px solid #ddd' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
              <button 
                ...
                style={{
                  padding: '6px 12px',
                  backgroundColor: currentZoom <= 1 ? '#ccc' : '#00a0ff',
                  color: 'white',
                  border: 'none',
                  borderRadius: '4px',
                  cursor: currentZoom <= 1 ? 'not-allowed' : 'pointer',
                  fontSize: '14px',
                  fontWeight: 'bold',
                  opacity: currentZoom <= 1 ? 0.5 : 1,
              ...
              <button 
                ...
                style={{
                  padding: '6px 12px',
                  backgroundColor: currentZoom >= 8 ? '#ccc' : '#00a0ff',
                  color: 'white',
                  border: 'none',
                  borderRadius: '4px',
                  cursor: currentZoom >= 8 ? 'not-allowed' : 'pointer',
                  fontSize: '14px',
                  fontWeight: 'bold',
                  opacity: currentZoom >= 8 ? 0.5 : 1,
                  transition: 'all 0.2s ease'
                }}
              >
                + Zoom In
          ...
          {selectedCountriesRef.current.size > 0 && (
            <div style={{ marginTop: '10px' }}>
              ...
              >
                Clear Selection ({selectedCountriesRef.current.size})
              </button>
            </div>
          )}
        </div>
