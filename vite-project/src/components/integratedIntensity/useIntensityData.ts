...
const DATASET_BY_INTENSITY_LEVEL = new Map<number, string>([
  [0, 'share-of-the-population-with-access-to-electricity'],
  [1, 'access-to-clean-fuels-and-technologies-for-cooking'],
  [2, 'share-of-individuals-using-the-internet'],
  [3, 'share-of-children-immunized-dtp3']
])
...
export function useIntensityData(intensityLevel: number): InternetDataResult {
  ...
  useEffect(() => {
    let cancelled = false
    const slug = DATASET_BY_INTENSITY_LEVEL.get(intensityLevel) ?? FALLBACK_SLUG
