...
export default function IntegratedIntensityBase({ intensityLevel }: IntegratedBaseProps) {
  const { loading, error, countries, years, valueDomain, metadata } = useIntensityData(intensityLevel)
  const preset = useMemo(() => getPreset(intensityLevel), [intensityLevel])
  const scaleAmp = useCallback((base: number) => base * preset.animMul, [preset.animMul])
  const scaleMs = useCallback((baseMs: number) => Math.round(baseMs * preset.timeMul), [preset.timeMul])
  const scaleCount = useCallback((base: number, cap = 999) => {
    const curved = Math.pow(preset.vfxMul, 1.35)
    return Math.min(cap, Math.max(0, Math.round(base * curved)))
  }, [preset.vfxMul])
  ...
  const hoverLayerSoundTimeoutRef = useRef<number | null>(null)
  const scrubLayerSoundTimeoutRef = useRef<number | null>(null)
  const selectLayerSoundTimeoutRef = useRef<number | null>(null)
  ...
  useEffect(() => {
    if (intensityLevel === 3) return
    clearTimeoutRef(hoverLayerSoundTimeoutRef)
    clearTimeoutRef(scrubLayerSoundTimeoutRef)
    clearTimeoutRef(selectLayerSoundTimeoutRef)
    clearTimeoutRef(reconfigLayerSoundTimeoutRef)
    clearTimeoutRef(encodeLayerSoundTimeoutRef)
    clearTimeoutRef(filterLayerSoundTimeoutRef)
  }, [intensityLevel])
  ...
  const triggerInBurst = useCallback((kind: InBurstKind, ms = 180) => {
    ...
    }, Math.max(1, scaleMs(ms)))
  }, [inOn, motionAllowed, scaleMs])
  ...
  const triggerPost = useCallback((kind: PostKind, options?: { duration?: number; badge?: PostBadge }) => {
    ...
    const duration = Math.max(1, scaleMs(options?.duration ?? 650))
  ...
  const playLocalAudio = useCallback((file: string, volume: number, useIntensityMul = false) => {
    try {
      ...
      const scaled = useIntensityMul ? volume * preset.audioMul : volume
      audio.volume = clamp(scaled, 0, 1)
      audio.currentTime = 0
  ...
  const arcs = useMemo(() => {
    ...
    return relatedItems
      .map((item, index) => {
        ...
        return { key: item.key, d: `M ${source.x} ${source.y} Q ${cx} ${cy} ${target.x} ${target.y}`, delay: scaleMs(index * 30), color: colorByRelated.get(item.key) ?? RELATED_PALETTE[index % RELATED_PALETTE.length] }
      })
      .filter((entry): entry is { key: string; d: string; delay: number; color: string } => entry !== null)
  }, [anchorFor, colorByRelated, relatedItems, scaleMs, selectedKey])
  ...
  const finalizeFilterPost = useCallback((options?: { count?: number; source?: FilterPostSource }) => {
    ...
    if (postOn && source === 'legend') {
      filterTimeoutRef.current = window.setTimeout(() => {
        ...
        }, Math.max(1, scaleMs(320)))
        filterTimeoutRef.current = null
      }, Math.max(1, scaleMs(800)))
      return
    }
    emitPost('filter_done', { count })
    triggerPost('filter', { duration: postOn ? 900 : 700, badge: { view: 'A', text: '\u2713 Filtered' } })
  }, [activeCount, emitPost, postOn, scaleMs, triggerPost])
  ...
  }, [finalizeFilterPost, inOn, motionAllowed, scaleMs, triggerFilterDone])
  ...
  const selectCountry = useCallback((key: string) => {
    ...
    if (postOn) {
      ...
      if (nextSelectedKey) {
        ...
        if (inOn) {
          postBridgeDelayTimeoutRef.current = window.setTimeout(readyPostBridge, Math.max(1, scaleMs(POST_PERSIST_BRIDGE_DELAY_MS)))
        } else {
          readyPostBridge()
        }
      }
    ...
    if (inOn && motionAllowed) {
      ...
      if (ds.length > 0) {
        ...
        }, Math.max(1, scaleMs(clickBridgeBurstMs)))
      } else {
        setClickBridge(null)
      }
    }
  ...
  const postPingLimit = scaleCount(6, 18)
  const postFrameLimit = scaleCount(10, 26)
  const fireworkCount = scaleCount(32, 96)
  const encodeSparkleRatio = clamp(preset.vfxMul, 0, 1)
  const connectPingPoints = useMemo(() => (
    relatedItems
      .slice(0, postPingLimit)
      .map((item, index) => {
        const anchor = anchorFor(item.key)
        if (!anchor) return null
        return { key: item.key, x: anchor.x, y: anchor.y, delay: scaleMs(index * 90) }
      })
      .filter((entry): entry is { key: string; x: number; y: number; delay: number } => entry !== null)
  ), [anchorFor, postPingLimit, relatedItems, scaleMs])
  const connectRowPingEntries = useMemo(() => (
    relatedItems
      ...
      .map((item, index) => {
        ...
        return { key: item.key, rowIndex, delay: scaleMs(index * 90) }
      })
      .filter((entry): entry is { key: string; rowIndex: number; delay: number } => entry !== null)
  ), [postPingLimit, relatedItems, scaleMs, topRowIndexByKey])
  const postSelectFrameEntries = useMemo(() => {
    ...
    return Array.from(uniqueByRow.entries())
      .sort((a, b) => a[0] - b[0])
      .slice(0, postFrameLimit)
      .map(([rowIndex, key], index) => ({ key, rowIndex, delay: scaleMs(index * 80) }))
  }, [postFrameLimit, relatedKeys, scaleMs, selectedKey, showPostSelectFx, topRowIndexByKey])
  const postFireworkParticles = useMemo(() => {
    if (!showPostSelectFx || !selectedAnchor || !motionAllowed) return []
    const count = fireworkCount
    if (count <= 0) return []
    return Array.from({ length: count }, (_, index) => {
      const jitter = Math.sin((postSelectFxNonce + 1) * 91 + index * 13) * scaleAmp(0.1)
      const angle = (Math.PI * 2 * index) / count + jitter
      const radius = scaleAmp(18 + (index % 8) * 4 + Math.sin(index * 7.3) * 3)
      ...
      return {
        ...
        dy,
        delay: 0,
        r: Math.max(0, scaleAmp(1.7 + (index % 4) * 0.23))
      }
    })
  }, [fireworkCount, motionAllowed, postSelectFxNonce, scaleAmp, selectedAnchor, showPostSelectFx])
  ...
  const shouldShowEncodeSparkle = useCallback((key: string) => {
    if (encodeSparkleRatio >= 1) return true
    if (encodeSparkleRatio <= 0) return false
    let hash = 0
    for (let index = 0; index < key.length; index += 1) {
      hash = (hash * 31 + key.charCodeAt(index)) % 997
    }
    return (hash % 1000) / 1000 < encodeSparkleRatio
  }, [encodeSparkleRatio])
  ...
  const renderBeeswarmLayer = (layerClassName: string, keyPrefix: string, pointerEvents: CSSProperties['pointerEvents'] = 'auto') => (
    <g className={layerClassName} pointerEvents={pointerEvents}>
      {beeswarmNodes.map((node, index) => {
        ...
        const sparkleDelay = scaleMs(Math.round(clamp(((node.x - leftX0) / Math.max(1, leftW)) * 420, 0, 420)))
        ...
        return (
          <g key={`${keyPrefix}-dot-${encodeFadeNonce}-${node.key}`}>
            <circle className={cls} cx={node.x} cy={node.y} r={dotRadius} style={{ '--ig-accent': accentFor(node.key), '--enter-delay': `${scaleMs(Math.min(260, index * 4))}ms`, '--sparkle-delay': `${sparkleDelay}ms` } as CSSProperties} onPointerDown={handleSelectPress} onMouseMove={event => handleCountryHover(event, node.key)} onMouseLeave={() => handleCountryLeave(node.key)} onClick={() => selectCountry(node.key)} />
  ...
  const renderHistogramLayer = (layerClassName: string, keyPrefix: string, pointerEvents: CSSProperties['pointerEvents'] = 'auto') => (
    <g className={layerClassName} pointerEvents={pointerEvents}>
      {histBars.map(bar => {
        ...
        return <rect key={`${keyPrefix}-bin-${encodeFadeNonce}-${bar.index}`} className={cls} x={bar.x0 + 1} y={bar.y} width={Math.max(1, bar.x1 - bar.x0 - 2)} height={Math.max(0, bar.h)} style={{ '--enter-delay': `${scaleMs(bar.index * 18)}ms` } as CSSProperties} onPointerDown={handleSelectPress} onMouseMove={event => handleBinMove(event, bar)} onMouseLeave={() => handleBinLeave(bar.index)} />
      })}
    </g>
  )
    ...
    <div className={`integrated-layout ${postOn ? 'post-on' : ''} ${postActiveClass} ${scrubbingClass} ${inActiveClass}`.trim()}>
      <div className="integrated-stage-wrap">
        ...
        >
          ...
          {!loading && !error && countries.length > 0 && (
            <>
              <svg ref={svgRef} className={`integrated-svg ${encodeSwitching ? 'is-encode-switching' : ''} ${isPostAbstract ? 'is-post-abstract' : ''} ${inOn && filterRegionFxActive ? 'is-filter-region-jitter' : ''}`} width={chartWidth} height={CHART_HEIGHT} viewBox={`0 0 ${chartWidth} ${CHART_HEIGHT}`} role="img" aria-label="Chart 9 integrated interactions">
                <defs>
                ...
                {showPostAbstractDetail2 && (
                  <g className="integrated-post-year-flash-layer" pointerEvents="none">
                    {yearTicks.map((tick, index) => (
                      <text key={`post-year-flash-${postNonce}-${tick}`} className="integrated-post-year-flash" x={xScaleYear(tick)} y={viewBBottom + 20} textAnchor="middle" style={{ '--tick-delay': `${scaleMs(index * 40)}ms` } as CSSProperties}>{tick}</text>
                    ))}
                  </g>
                )}
                ...
                <g className="integrated-markers-layer">
                  ...
                  {!postOn && isPostSelect && selectedMarkerY !== null && selectedKey && (
                    <circle key={`halo-${postNonce}-${selectedKey}`} cx={focusX} cy={selectedMarkerY} r={scaleAmp(7.2)} className="integrated-post-select-halo" style={{ '--ig-accent': accentFor(selectedKey) } as CSSProperties} />
                  )}
                </g>
                ...
                {!postOn && isPostSelect && selectedAnchor && (
                  <circle key={`post-select-anchor-${postNonce}`} className="integrated-post-select-pulse" cx={selectedAnchor.x} cy={selectedAnchor.y} r={scaleAmp(7.1)} />
                )}
